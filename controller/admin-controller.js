...
module.exports = {
    ...
    changeStatus: async (req, res) => {
        ...
        if (status == 'Order Confirmed') {
            await orderModerl.findOneAndUpdate({ _id: orderId, 'products.product': proId },
                {
                    $set:
...
printBill: async(req,res)=>{
    ...
    const order = await orderModerl.findOne({_id:orderId}).populate('products.product').populate('userId')
